...
public:
    ...
    // Runtime: O(1)
    explicit UnorderedFastPQ(COMP_FUNCTOR comp = COMP_FUNCTOR()) :                      //Default constructor?
    ...
    UnorderedFastPQ(InputIterator start, InputIterator end, COMP_FUNCTOR comp = COMP_FUNCTOR()) :              // Normal constructor
        BaseClass{ comp }, data{ start, end }, extreme{ UNKNOWN } {                                             //Why there is no implementation at all?
    } // UnorderedFastPQ()
    ...
    // Runtime: O(1)
    virtual void updatePriorities() {                                           //                              What does this mean
        extreme = UNKNOWN;
    } // updatePriorities()
    ...
    virtual void push(const TYPE &val) {
        ...
        // Since a new element has been added, we no longer know where to find                          // Does this mean, when ever we push a new element
                                                                                                        // We need to find the extreme again?
